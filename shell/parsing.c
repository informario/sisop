...
expand_environ_var(char *arg)
{
	...
	if (strstr(arg, "$") && !strstr(arg, "$?")) {
		...
		if (env == NULL) {
			printf_debug("Error obteniendo la variable de "
			             "ambiente: %s\n",
			             strerror(errno));
			strcpy(arg, " ");
			return arg;
		...
		strncpy(argExpanded, env, strlen(env));

		argExpanded[strlen(argExpanded)] = '\0';
	...
	// variable de entorno mágica o pseudo-variable
	if (strstr(arg, "$?")) {
