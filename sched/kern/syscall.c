...
static int
sys_set_priority(uint32_t new_priority)
{
	int cur_env_priority = sys_get_priority();
	if (new_priority > LOWEST_PRIORITY || new_priority < HIGHEST_PRIORITY || new_priority < cur_env_priority){
		cprintf("old: %d, new:%d\n");
		return E_INVAL;
	}
